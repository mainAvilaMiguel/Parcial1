...
public class AnimalService {
    ...
    public List<Animal_number> getNumberByCategory() throws IOException {
        List<String> listAnimal = Files.readAllLines(Paths.get(filePath));
        List<Animal_number> animales = new ArrayList<>();
        ...
        for (String line : listAnimal) {
            ...
            if (parts.length == 2) {
                String category = parts[0].trim();
                int counter = 0;
                for (String line2 : listAnimal) {
                    if (line2.split(",")[0].trim().equals(category)) {
                        counter++;
                    }
                }
                if (animales.stream().noneMatch(a -> a.getCategory().equals(category))) {
                    animales.add(new Animal_number(counter,category));
                }
            }
        }
        return animales;
